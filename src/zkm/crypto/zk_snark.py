"""
Bulletproofs-based Zero-Knowledge Proof Implementation
Compatible with Zerocash and Morales et al. papers

Bulletproofs are:
- Succinct (short proofs)
- Non-interactive
- Based on discrete log
- Suitable for payment proofs
"""
...
class ZKSNARKProof:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "relinkeability_evidence": bytes_to_hex(self.relinkeability_evidence) if self.relinkeability_evidence else None,
    ...
    def from_dict(data: Dict[str, Any]) -> 'ZKSNARKProof':
        ...
        return ZKSNARKProof(
            ...
            trapdoor_proof=hex_to_bytes(data["trapdoor_proof"]) if data.get("trapdoor_proof") else None,
            relinkeability_evidence=hex_to_bytes(data["relinkeability_evidence"]) if data.get("relinkeability_evidence") else None,
...
class BulletproofZKProver:
    ...
    def create_nullifier_proof(
        ...
        # Create challenge
        challenge_input = nullifier + R.x.to_bytes(32, byteorder='big')
        challenge = self._hash_to_scalar(challenge_input)
        ...
        # Package proof
        proof = (
            R.x.to_bytes(32, byteorder='big') +
            R.y.to_bytes(32, byteorder='big') +
    ...
    def create_value_proof(
        ...
        commitment = (value_scalar * self.G + bf_scalar * self.key.G)
        
        # Create proof of equality
        challenge_input = commitment.x.to_bytes(32, byteorder='big')
        challenge = self._hash_to_scalar(challenge_input)
        ...
        proof = (
            commitment.x.to_bytes(32, byteorder='big') +
            commitment.y.to_bytes(32, byteorder='big') +
            R.x.to_bytes(32, byteorder='big') +
            R.y.to_bytes(32, byteorder='big') +
            z.to_bytes(32, byteorder='big')
        )
    ...
    def create_output_proof(
        ...
        challenge_input = output_commitment + R.x.to_bytes(32, byteorder='big')
        challenge = self._hash_to_scalar(challenge_input)
    ...
    def create_complete_payment_proof(
        ...
        nullifier_proof = self.create_nullifier_proof(
            nullifier, commitment_secret
        )
        
        value_proof = self.create_value_proof(
            input_amount, output_amount, randomness
        )
        ...
        output_proof = self.create_output_proof(
            output_commitment, spend_key, randomness
        )
        ...
        if include_trapdoor and auditor_public_key:
            trapdoor_proof = self._create_trapdoor_proof(
...
class ZKSNARKVerifier:
    ...
    def verify_payment_proof(
        ...
        try:
            ...
            if not self.prover.verify_commitment_proof(
