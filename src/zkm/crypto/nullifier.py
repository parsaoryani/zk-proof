...
Per Ben-Sasson et al., 2014:
  - Nullifier sn = Hash_sn(a_sk, rho) prevents double-spending
  - Cannot be linked to commitment (commitment = Hash_cm(a_pk, rho, v))
  - Completely separates deposit phase from withdrawal phase
  - Enables universal relink prevention
"""
...
class NullifierSet:
    ...
    def register(self, nullifier: str, transaction_hash: str,
                 spent_by_user: Optional[str] = None,
                 merkle_root: Optional[str] = None) -> bool:
        """
        ...
        record = NullifierRecord(
            ...
            spent_at=datetime.utcnow().isoformat() + "Z",
    ...
    def serialize(self) -> str:
        ...
        records_dict = {
            nullifier: json.loads(record.serialize())
            for nullifier, record in self.records.items()
        }
        
        return json.dumps({
            "nullifiers": list(self.nullifiers),
            "records": records_dict,
            "total_spent": self.size
        })
