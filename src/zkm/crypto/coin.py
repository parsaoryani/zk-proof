"""Zerocash-compliant coin implementation with proper serialization.

Implements core coin structures from Ben-Sasson et al. 2014 (Zerocash: Decentralized
Anonymous Payments from Bitcoin). Provides coin generation, commitment computation,
and nullifier management for the zero-knowledge privacy system.
...
Coin Structure:
    A coin c = (k, r, v, rho) where:
    - k = spend key (secret, unique per coin)
    - r = randomness (hides commitment structure)
    - v = value (amount in the coin)
    - rho = rho (serial number for nullifier)

Commitment:
    cm = COMM_r(k || v) = SHA-256(k || r || v)

Nullifier:
    sn = PRF_k(rho) = SHA-256(k || rho)

Example Usage:
    >>> from zkm.crypto.coin import Coin, CoinStatus
    >>>
    >>> # Generate new coin
    >>> coin = Coin.generate(value=1000)
    >>> print(f"Commitment: {coin.commitment[:16]}...")
    >>>
    >>> # Compute nullifier for withdrawal
    >>> nullifier = coin.compute_nullifier()
    >>> print(f"Nullifier: {nullifier[:16]}...")
    >>>
    >>> # Check if coin can be spent
    >>> if coin.is_spendable():
    ...     coin.mark_spent()
    ...     print(f"Coin status: {coin.status}")

Security Notes:
    - Spend keys (k) must be kept secret to prevent coin theft
    - Nullifiers reveal double-spend attempts but not identity
    - Commitments are perfectly hiding under SHA-256 collision resistance
    - Never reuse randomness (r) across different coins
"""
...
class Coin:
    ...
    def compute_commitment(self) -> str:
        """
        Public method to compute commitment.
        
        Returns the already-computed commitment or recomputes if needed.
        This is the public interface expected by tests and external code.
        """
        if self.commitment:
    ...
    def is_spendable(self) -> bool:
        """Check if coin can be spent."""
        return self.status == CoinStatus.ACTIVE and self.merkle_index is not None

    def mark_spent(self):
        """Mark coin as spent after successful withdrawal."""
        self.status = CoinStatus.SPENT
        self.spent_at = datetime.now(UTC).isoformat() + "Z"
