...
from zkm.security import hash_password, verify_password, create_access_token, verify_access_token, generate_random_token
...
async def get_current_user(authorization: Optional[str] = Header(None), db: DatabaseManager = Depends(get_db_manager)):
    """
...
def register_auth_routes(app: FastAPI):
    ...
    async def logout(current_user = Depends(get_current_user), db: DatabaseManager = Depends(get_db_manager)):
        """
