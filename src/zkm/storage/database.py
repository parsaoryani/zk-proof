"""Database abstraction layer for Zerocash mixer.

Provides persistent storage for Zerocash state across multiple layers:

Core Tables:
    - Coins: Commitment hashes and metadata
    - Nullifiers: Spent coin identifiers (double-spend prevention)
    - Merkle Roots: Historic Merkle tree roots (proof validity validation)
    - Transactions: Deposit and withdrawal records
    - Identity Proofs: KYC/AML audit trail (encrypted)
...
Optimization:
    - Indexed lookups for nullifier existence checks
    - Efficient range queries on timestamps
    - Merkle tree node caching
    - Connection pooling for throughput

Security:
    - All sensitive data encrypted at rest
    - Identity proofs encrypted with auditor keys
    - Immutable audit trail
    - Role-based access control (RBAC)

Example:
    >>> from zkm.storage.database import get_db
    >>>
    >>> db = get_db()
    >>>
    >>> # Check if nullifier already spent
    >>> is_spent = db.is_nullifier_spent(nullifier_hash)
    >>> if not is_spent:
    ...     db.record_nullifier(nullifier_hash, tx_id)
    ... else:
    ...     raise DoubleSpendError()
    >>>
    >>> # Store Merkle root for verification window
    >>> db.store_merkle_root(root_hash, height)
"""
...
class User(Base):
    ...
    balance = Column(Float, default=0.0, nullable=False)  # User's current balance
...
class UserSession(Base):
    ...
    def is_valid(self) -> bool:
        ...
        return self.expires_at > datetime.now(UTC) and self.user.is_active
...
class Commitment(Base):
    ...
    commitment_index = Column(
        Integer, nullable=False, index=True
    )  # Not unique - multiple deposits may use same index
...
class AuditRecord(Base):
    ...
    )  # Allows multiple audits per transaction
    transaction_hash = Column(String(255), nullable=False, index=True)
...
class DatabaseManager:
    ...
    def update_transaction_status(
        self, session: Session, transaction_hash: str, status: TransactionStatus
    ) -> bool:
    ...
    def get_user_transactions(
        self, session: Session, user_id: int, limit: int = 20
    ) -> List[Transaction]:
    ...
    def update_user_balance(self, session: Session, user_id: int, amount: float) -> bool:
        """Update user's balance by adding the specified amount (positive for deposit, negative for withdrawal)."""
        user = session.query(User).filter_by(id=user_id).first()
        if user:
    ...
    def get_total_balance(self, session: Session) -> float:
        """Get total balance across all users."""
        from sqlalchemy import func
        result = session.query(func.sum(User.balance)).scalar()
    ...
    def get_session_by_token(self, session: Session, token: str) -> Optional[UserSession]:
        ...
        return session.query(UserSession).filter_by(token=token).first()
    ...
    def invalidate_session(self, session: Session, token: str) -> bool:
        ...
        if sess:
            sess.expires_at = datetime.now(UTC)
            session.commit()
            return True
        return False
    ...
    def invalidate_user_sessions(self, session: Session, user_id: int) -> bool:
        ...
        sessions = session.query(UserSession).filter_by(user_id=user_id).all()
    ...
    def add_commitment(
        ...
        encrypted_randomness: bytes,
        amount: float,
        encrypted_identity: Optional[bytes] = None,
    ) -> Commitment:
    ...
    def get_commitment_by_index(
        self, session: Session, commitment_index: int
    ) -> Optional[Commitment]:
    ...
    def get_commitment_by_hash(
        self, session: Session, commitment_hash: bytes
    ) -> Optional[Commitment]:
        """Get commitment by hash."""
        return session.query(Commitment).filter_by(commitment_hash=commitment_hash).first()

    def get_commitment_by_transaction(
        self, session: Session, transaction_hash: str
    ) -> Optional[Commitment]:
    ...
    def add_nullifier(
        self, session: Session, nullifier_hash: bytes, commitment_hash: bytes
    ) -> Nullifier:
        """Add nullifier record."""
        nullifier = Nullifier(
            nullifier_hash=nullifier_hash, commitment_hash=commitment_hash, is_spent=0
        )
        session.add(nullifier)
        session.commit()
        return nullifier

    def mark_nullifier_spent(
        self, session: Session, nullifier_hash: bytes, withdrawal_hash: str
    ) -> bool:
        ...
        if nullifier and not nullifier.is_spent:
            ...
            nullifier.spent_timestamp = datetime.now(UTC)
    ...
    def get_audits_for_transaction(
        self, session: Session, transaction_hash: str
    ) -> List[AuditRecord]:
    ...
    def add_merkle_root(
        self, session: Session, root_hash: bytes, tree_height: int, num_leaves: int
    ) -> MerkleRoot:
        """Add Merkle root snapshot."""
        root = MerkleRoot(root_hash=root_hash, tree_height=tree_height, num_leaves=num_leaves)
        session.add(root)
        session.commit()
        return root
    ...
    def get_transaction_count(
        self, session: Session, tx_type: Optional[TransactionType] = None
    ) -> int:
