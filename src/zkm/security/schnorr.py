...
Implements Schnorr's protocol (Schnorr, 1989) for proving knowledge of
discrete logarithm without revealing the secret. Used in Zerocash for:
    - Commitment opening proofs
    - Spend authority validation
    - Digital signatures

Mathematical Basis:
    Given public key PK = g^sk (mod p) where:
    - g = generator
    - sk = secret key (unknown to verifier)
    - p = large prime

    Schnorr proves knowledge of sk without revealing it:
    1. Prover chooses random r, computes R = g^r
    2. Prover computes challenge c = Hash(g || PK || R)
    3. Prover computes response z = r + c*sk
    4. Verifier checks: g^z = R * PK^c

Paper Reference:
    - Schnorr, C. P. (1989). "Efficient Identification and Signatures for Smart Cards"
    - Zerocash uses Schnorr-like proofs for commitment openings

Properties:
    - Soundness: Attacker cannot forge proof without sk
    - Zero-Knowledge: Verifier learns nothing about sk
    - Non-Interactive: Fiat-Shamir enables single-message proofs
    - Efficient: Computation linear in security parameter

Example Usage:
    >>> from zkm.security.schnorr import SchnorrProver, SchnorrVerifier
    >>>
    >>> # Prover generates public/secret keys
    >>> prover = SchnorrProver()
    >>> public_key = prover.public_key
    >>>
    >>> # Prover creates proof of knowledge
    >>> proof = prover.prove(message=b"coin_commitment")
    >>>
    >>> # Verifier checks proof
    >>> verifier = SchnorrVerifier(public_key)
    >>> is_valid = verifier.verify(proof, message=b"coin_commitment")
    >>> assert is_valid
"""
...
class BulletproofZKProof:
    ...
    def _pedersen_commit(self, value: int, blinding: int) -> bytes:
        ...
        G = key_g.G  # type: ignore[attr-defined]
    ...
    def _range_proof_helper(
        self, value: int, blinding: int, bit_length: int = 64
    ) -> Dict[str, Any]:
        """
        ...
        for i in range(bit_length):
            ...
            proof_elements.append({"bit": bit, "commitment": bytes_to_hex(bit_commit)})

        # Challenge and response (Fiat-Shamir heuristic)
        challenge_input = commitment + b"".join(
            [bytes.fromhex(pe["commitment"]) for pe in proof_elements]
        )
    ...
    def generate_withdrawal_proof(
        self, commitment_secret: int, nullifier: bytes, identity: str, amount: int = 0
        ...
        public_key = key.d * key.G  # type: ignore[attr-defined]
        public_key_bytes = public_key.x.to_bytes(32, byteorder="big") + public_key.y.to_bytes(
            32, byteorder="big"
        )
        ...
        dlog_commitment = secrets.randbelow(self.order) * key.G  # type: ignore[attr-defined]
    ...
    def verify_withdrawal_proof(self, proof_bytes: bytes, nullifier: bytes, identity: str) -> bool:
        """
        ...
        try:
            ...
            if (datetime.now(UTC) - proof_time) > timedelta(hours=1):
                return False
